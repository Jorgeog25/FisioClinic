...
export default function ClientHome() {
  ...
  useEffect(() => { api.myHistory().then(setHistory).catch(()=>{}) }, [])
    ...
    <>
      ...
      <div className="row">
        <div className="card">
          ...
          {date && (
            <>
              <p style={{marginTop:10}}>Horas disponibles el <strong>{date}</strong></p>
              <div className="slots">
                {(!Array.isArray(slots) || slots.length === 0) && (
                  <p style={{opacity:.7}}>No hay horas disponibles.</p>
                )}
                {Array.isArray(slots) && slots.map((s,i)=>(
                  <div key={`${date}-${s}-${i}`} className="slot free" onClick={()=>bookSlot(s)}>{s}</div>
                ))}
              </div>
