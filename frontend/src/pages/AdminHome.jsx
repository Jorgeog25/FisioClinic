...
export default function AdminHome() {
  ...
  function logout(){ clearAuth(); nav('/', { replace:true }) }

  useEffect(()=>{ if (tab==='clients') loadClients() }, [tab])
  async function loadClients(){ const list = await api.listClients(); setClients(list) }

  function pickDay(d){ setDate(d); setMsg(''); setShowDayForm(true); setAppts([]) }
  ...
  function startEdit(c){
    setEditingId(c._id)
    setEditForm({ firstName:c.firstName||'', lastName:c.lastName||'', phone:c.phone||'', reason:c.reason||'' })
  }
  function cancelEdit(){ setEditingId(null); setEditForm({ firstName:'', lastName:'', phone:'', reason:'' }) }
  ...
  async function removeClient(id){
    const ok = confirm('¿Seguro que deseas borrar este cliente? También se eliminarán sus citas y pagos.')
    if (!ok) return
    try { await api.deleteClient(id); setClients(clients.filter(c=>c._id!==id)) }
    catch(e){ alert(e.message) }
  }
  ...
  return (
    <>
      ...
      <div className="nav" style={{marginTop:8}}>
        <button className={`btn ${tab==='calendar'?'primary':''}`} onClick={()=>setTab('calendar')}>Calendario</button>
        <button className={`btn ${tab==='clients'?'primary':''}`} onClick={()=>setTab('clients')}>Ver clientes</button>
      </div>
          ...
          <div className="card">
            ...
            {date && (
              <div style={{marginTop:12, display:'flex', gap:8, flexWrap:'wrap'}}>
                <span>Día seleccionado: <strong>{date}</strong></span>
                <button className="btn" onClick={viewDayAppointments}>Ver citas del día</button>
                <button className="btn" onClick={()=>setShowDayForm(v=>!v)}>{showDayForm?'Cerrar día':'Abrir día'}</button>
              </div>
            )}
            ...
            {appts.length>0 && (
              <div style={{marginTop:12}}>
                ...
                <table>
                  <thead>
                    <tr><th>Hora</th><th>Cliente</th><th>Motivo</th><th>Estado</th></tr>
                  </thead>
                  <tbody>
                    {appts.map(a=>(
                      <tr key={a._id}>
                        <td>{a.time}</td>
                        <td>{a.clientId ? `${a.clientId.firstName} ${a.clientId.lastName}` : '—'}</td>
          ...
          {showDayForm && (
            <div className="card">
              ...
              <form onSubmit={saveDay}>
                ...
                <div className="row">
                  <div><label>Duración (min)</label><input type="number" value={slotMinutes} onChange={e=>setSlotMinutes(e.target.value)} /></div>
                  <div>
                    <label>Activo</label>
      ...
      {tab === 'clients' && (
        <div className="card">
          ...
          <table style={{marginTop:12}}>
            <thead>
              <tr><th>Nombre</th><th>Teléfono</th><th>Motivo</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              {clients.map(c=>{
                ...
                return (
                  <tr key={c._id}>
                    ...
                    <td>{isEditing ? <input value={editForm.phone} onChange={e=>setEditForm({...editForm, phone:e.target.value})}/> : c.phone}</td>
                    <td>{isEditing ? <input value={editForm.reason} onChange={e=>setEditForm({...editForm, reason:e.target.value})}/> : (c.reason || '—')}</td>
                    <td>
                      {!isEditing ? (
                        <div style={{display:'flex', gap:8}}>
                          <button className="btn" onClick={()=>startEdit(c)}>Editar</button>
                          <button className="btn" onClick={()=>removeClient(c._id)}>Borrar</button>
                        </div>
                      ) : (
                        <div style={{display:'flex', gap:8}}>
                          <button className="btn primary" disabled={savingEdit} onClick={()=>saveEdit(c._id)}>Guardar</button>
